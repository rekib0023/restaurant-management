...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		projectStage := bson.D{
			{
				"$project", bson.D{
					{"_id", 0},
					{"total_count", 1},
					{"food_items", bson.D{{"$slice", []interface{}{"$data", offset, limit}}}},
				},
			},
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while trying to list all the food items"})
			return
		}
...
func GetFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error occured while fecthing the food item"})
		}
